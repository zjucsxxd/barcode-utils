 ...
 *   a) system bus service - to provide settings to BarcodeManager and other
          ...
          const char *detail)
{
	...
	if (!gc) {
		bm_warning ("Could not get the %s bus.  Make sure the message "
	...
	*proxy = dbus_g_proxy_new_for_name (gc,
	                                    ...
	                                    "org.freedesktop.DBus");
	if (!*proxy) {
		bm_warning ("Could not get the DBus object!");
...
request_name (DBusGProxy *proxy, int flags, const char *detail)
{
	...
	if (!dbus_g_proxy_call (proxy, "RequestName", &error,
	                        G_TYPE_STRING, BM_DBUS_SERVICE_USER_SETTINGS,
	                        G_TYPE_UINT, flags,
	                        G_TYPE_INVALID,
	                        G_TYPE_UINT, &request_name_result,
	                        G_TYPE_INVALID)) {
		bm_warning ("Could not acquire the %s service.\n"
		            "  Error: (%d) %s",
		            detail,
		            error ? error->code : -1,
	...
	if (request_name_result != DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER) {
		if (request_name_result == DBUS_REQUEST_NAME_REPLY_EXISTS)
			fprintf (stdout, _("An instance of bm-applet is already running.\n"));
		else {
...
system_bus_destroy_cb (DBusGProxy *proxy, gpointer user_data)
{
	...
	bm_info ("disconnected by the system bus.");
...
session_bus_destroy_cb (DBusGProxy *proxy, gpointer user_data)
{
	...
	bm_info ("disconnected by the session bus.");
...
session_bus_init (AppletDBusManager *self)
{
	...
	if (priv->session_g_connection) {
		bm_warning ("session connection already exists");
		return FALSE;
	}
...
reconnect_cb (gpointer user_data)
{
	...
	if (priv->g_connection)
		...
		if (system_bus_init (self)) {
			if (applet_dbus_manager_start_service (self)) {
				bm_info ("reconnected to the system bus.");
	...
	if (priv->session_g_connection)
		...
		if (session_bus_init (self)) {
			bm_info ("reconnected to the session bus.");
                                    ...
                                    const char *name)
{
	...
	if (!dbus_g_proxy_call (APPLET_DBUS_MANAGER_GET_PRIVATE (self)->proxy,
	                        ...
	                        G_TYPE_STRING, &owner,
	                        G_TYPE_INVALID)) {
		bm_warning ("Error on GetNameOwner DBUS call: %s", err->message);
		g_error_free (err);
	}
                                    ...
                                    const char *name)
{
	...
	if (!dbus_g_proxy_call (APPLET_DBUS_MANAGER_GET_PRIVATE (self)->proxy,
	                        ...
	                        G_TYPE_INVALID)) {
		bm_warning ("Error on NameHasOwner DBUS call: %s", err->message);
		g_error_free (err);
	}
...
system_bus_init (AppletDBusManager *self)
{
	...
	if (priv->connection) {
		bm_warning ("DBus Manager already has a valid connection.");
		return FALSE;
	}
...
applet_dbus_manager_class_init (AppletDBusManagerClass *klass)
{
	...
	signals[CONNECTION_CHANGED] =
		g_signal_new ("dbus-connection-changed",
		              ...
		              NULL, NULL, bma_marshal_VOID__POINTER,
	...
	signals[NAME_OWNER_CHANGED] =
		g_signal_new ("name-owner-changed",
		              ...
		              NULL, NULL, bma_marshal_VOID__STRING_STRING_STRING,
