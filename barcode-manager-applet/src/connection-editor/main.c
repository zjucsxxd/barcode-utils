...
#define BM_CE_DBUS_SERVICE_NAME "org.freedesktop.BarcodeManager.Gnome.ConnectionEditor"

#define BM_TYPE_CE_SERVICE            (bm_ce_service_get_type ())
#define BM_CE_SERVICE(obj)            (G_TYPE_CHECK_INSTANCE_CAST ((obj), BM_TYPE_CE_SERVICE, NMCEService))
#define BM_CE_SERVICE_CLASS(klass)    (G_TYPE_CHECK_CLASS_CAST ((klass), BM_TYPE_CE_SERVICE, NMCEServiceClass))
#define BM_IS_CE_SERVICE(obj)         (G_TYPE_CHECK_INSTANCE_TYPE ((obj), BM_TYPE_CE_SERVICE))
#define BM_IS_CE_SERVICE_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE ((obj), BM_TYPE_CE_SERVICE))
#define BM_CE_SERVICE_GET_CLASS(obj)  (G_TYPE_INSTANCE_GET_CLASS ((obj), BM_TYPE_CE_SERVICE, NMCEServiceClass))
...
GType bm_ce_service_get_type (void);

G_DEFINE_TYPE (NMCEService, bm_ce_service, G_TYPE_OBJECT)

static gboolean impl_start (NMCEService *self, GHashTable *args, GError **error);

#include "bm-connection-editor-service-glue.h"
...
static void
bm_ce_service_init (NMCEService *self)
{
}

static void
bm_ce_service_class_init (NMCEServiceClass *config_class)
{
	dbus_g_object_type_install_info (G_TYPE_FROM_CLASS (config_class),
									 &dbus_glib_bm_connection_editor_service_object_info);
}
...
impl_start (NMCEService *self, GHashTable *table, GError **error)
{
	...
	// FIXME bm_connection_list_present (self->list);
...
try_existing_instance (DBusGConnection *bus, DBusGProxy *proxy, const char *type)
{
	...
	if (!dbus_g_proxy_call (proxy, "NameHasOwner", NULL,
	                        G_TYPE_STRING, BM_CE_DBUS_SERVICE_NAME, G_TYPE_INVALID,
...
main (int argc, char *argv[])
{
	...
	GOptionEntry entries[] = {
		{ ARG_TYPE, 0, 0, G_OPTION_ARG_STRING, &type, "Type of connection to show at launch", BM_SETTING_WIRED_SETTING_NAME },
		{ NULL }
		};*/
	...
	/*
	if (!type)
		type = (char *) BM_SETTING_WIRED_SETTING_NAME;
	if (!strcmp (type, BM_SETTING_CDMA_SETTING_NAME))
		type = (char *) BM_SETTING_GSM_SETTING_NAME;
	*/

	ctype = bm_connection_lookup_setting_type (type);
	// FIXME list = bm_connection_list_new (ctype);
	list = NULL;
	if (!list) {
