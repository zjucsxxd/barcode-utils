...
typedef struct {
	...
	BMSettingsConnectionInterface *pan_connection;

	gboolean dun;
	GtkWidget *dun_button;
	guint dun_toggled_id;
	BMSettingsConnectionInterface *dun_connection;
...
add_pan_connection (PluginInfo *info)
{
	...
	BMSetting *setting, *bt_setting, *ip_setting;
	BMAGConfSettings *gconf_settings;
	BMAGConfConnection *exported = NULL;
	GByteArray *mac;
	...
	setting = bm_setting_connection_new ();
	id = g_strdup_printf (_("%s Network"), alias ? alias : info->bdaddr);
	uuid = bm_utils_uuid_generate ();
	g_object_set (G_OBJECT (setting),
	              ...
	              BM_SETTING_CONNECTION_AUTOCONNECT, FALSE,
	              NULL);
	g_free (id);
	g_free (uuid);
	...
	bt_setting = bm_setting_bluetooth_new ();
	g_object_set (G_OBJECT (bt_setting),
	              BM_SETTING_BLUETOOTH_BDADDR, mac,
	              BM_SETTING_BLUETOOTH_TYPE, BM_SETTING_BLUETOOTH_TYPE_PANU,
	              NULL);
	bm_connection_add_setting (connection, bt_setting);

	/* The IPv4 settings */
	ip_setting = bm_setting_ip4_config_new ();
	g_object_set (G_OBJECT (ip_setting),
	              BM_SETTING_IP4_CONFIG_METHOD, BM_SETTING_IP4_CONFIG_METHOD_AUTO,
	              NULL);
	bm_connection_add_setting (connection, ip_setting);
...
out:
	...
	g_free (alias);
	return exported ? BM_SETTINGS_CONNECTION_INTERFACE (exported) : NULL;
}
...
dun_new_cdma (MobileWizardAccessMethod *method)
{
	...
	setting = bm_setting_cdma_new ();
	g_object_set (setting,
	              BM_SETTING_CDMA_NUMBER, "#777",
	              BM_SETTING_CDMA_USERNAME, method->username,
	              BM_SETTING_CDMA_PASSWORD, method->password,
	              NULL);
	bm_connection_add_setting (connection, setting);

	/* Serial setting */
	setting = bm_setting_serial_new ();
	g_object_set (setting,
	              BM_SETTING_SERIAL_BAUD, 115200,
	              BM_SETTING_SERIAL_BITS, 8,
	              BM_SETTING_SERIAL_PARITY, 'n',
	              BM_SETTING_SERIAL_STOPBITS, 1,
	              NULL);
	bm_connection_add_setting (connection, setting);

	bm_connection_add_setting (connection, bm_setting_ppp_new ());
...
dun_new_gsm (MobileWizardAccessMethod *method)
{
	...
	setting = bm_setting_gsm_new ();
	g_object_set (setting,
	              BM_SETTING_GSM_NUMBER, "*99#",
	              BM_SETTING_GSM_USERNAME, method->username,
	              BM_SETTING_GSM_PASSWORD, method->password,
	              BM_SETTING_GSM_APN, method->gsm_apn,
	              NULL);
	bm_connection_add_setting (connection, setting);
...
static void
delete_cb (BMSettingsConnectionInterface *connection,
           GError *error,
           gpointer user_data)
{
	if (error) {
...
bm_is_running (void)
{
	...
	if (!dbus_g_proxy_call (proxy, "NameHasOwner", &error,
	                        ...
	                        G_TYPE_INVALID)) {
		g_message (_("Bluetooth configuration not possible (error finding BarcodeManager: %s)."),
		           error && error->message ? error->message : "unknown");
	}
...
device_removed (const char *bdaddr)
{
	...
	settings = BM_SETTINGS_INTERFACE (nma_gconf_settings_new (NULL));

	/* First PAN */
	do {
		connection = get_connection_for_bdaddr (settings, bdaddr, TRUE);
		if (connection)
			bm_settings_connection_interface_delete (connection, delete_cb, NULL);
	} while (connection);
