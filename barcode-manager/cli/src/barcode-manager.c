/* nmcli - command-line tool to control BarcodeManager
 *
...
static NmcOutputField nmc_fields_bm_status[] = {
...
#define NMC_FIELDS_BM_STATUS_ALL     "RUNNING,STATE,NET-ENABLED,WIFI-HARDWARE,WIFI,WWAN-HARDWARE,WWAN"
#define NMC_FIELDS_BM_STATUS_COMMON  "RUNNING,STATE,WIFI-HARDWARE,WIFI,WWAN-HARDWARE,WWAN"
#define NMC_FIELDS_BM_NET_ENABLED    "NET-ENABLED"
#define NMC_FIELDS_BM_WIFI           "WIFI"
#define NMC_FIELDS_BM_WWAN           "WWAN"
...
static NMCResultCode show_bm_status (NmCli *nmc);
...
show_bm_status (NmCli *nmc)
{
	...
	const char *fields_all =    NMC_FIELDS_BM_STATUS_ALL;
	const char *fields_common = NMC_FIELDS_BM_STATUS_COMMON;
	...
	nmc->allowed_fields = nmc_fields_bm_status;
	...
	if (error) {
		if (error->code == 0)
			...
			g_string_printf (nmc->return_text, _("Error: 'nm status': %s; allowed fields: %s"), error->message, NMC_FIELDS_BM_STATUS_ALL);
		g_error_free (error);
	...
	nmc->print_fields.header_name = _("BarcodeManager status");
	print_fields (nmc->print_fields, nmc->allowed_fields); /* Print header */

	bm_running = nmc_is_bm_running (nmc, NULL);
	if (bm_running) {
		nmc->get_client (nmc); /* create BMClient */
		state = bm_client_get_state (nmc->client);
		net_enabled_str = bm_client_networking_get_enabled (nmc->client) ? _("enabled") : _("disabled");
		wireless_hw_enabled_str = bm_client_wireless_hardware_get_enabled (nmc->client) ? _("enabled") : _("disabled");
		wireless_enabled_str = bm_client_wireless_get_enabled (nmc->client) ? _("enabled") : _("disabled");
		wwan_hw_enabled_str = bm_client_wwan_hardware_get_enabled (nmc->client) ? _("enabled") : _("disabled");
		wwan_enabled_str = bm_client_wwan_get_enabled (nmc->client) ? _("enabled") : _("disabled");
	} else {
		net_enabled_str = wireless_hw_enabled_str = wireless_enabled_str = wwan_hw_enabled_str = wwan_enabled_str = _("unknown");
	}

	nmc->allowed_fields[0].value = bm_running ? _("running") : _("not running");
	nmc->allowed_fields[1].value = bm_state_to_string (state);
...
do_network_manager (NmCli *nmc, int argc, char **argv)
{
	...
	if (argc == 0) {
		if (!nmc_terse_option_check (nmc->print_output, nmc->required_fields, &error))
			goto opt_error;
		nmc->return_value = show_bm_status (nmc);
	}
		...
		if (matches (*argv, "status") == 0) {
			...
			if (next_arg (&argc, &argv) != 0) {
				...
				if (nmc->required_fields && strcasecmp (nmc->required_fields, "NET-ENABLED")) {
					g_string_printf (nmc->return_text, _("Error: '--fields' value '%s' is not valid here; allowed fields: %s"),
					                 nmc->required_fields, NMC_FIELDS_BM_NET_ENABLED);
					nmc->return_value = NMC_RESULT_ERROR_USER_INPUT;
					goto end;
				}
				nmc->allowed_fields = nmc_fields_bm_status;
				nmc->print_fields.indices = parse_output_fields (NMC_FIELDS_BM_NET_ENABLED, nmc->allowed_fields, NULL);
				nmc->print_fields.flags = multiline_flag | mode_flag | escape_flag | NMC_PF_FLAG_MAIN_HEADER_ADD | NMC_PF_FLAG_FIELD_NAMES;
				nmc->print_fields.header_name = _("Networking enabled");
				print_fields (nmc->print_fields, nmc->allowed_fields); /* Print header */

				if (nmc_is_bm_running (nmc, NULL)) {
					nmc->get_client (nmc); /* create BMClient */
					nmc->allowed_fields[2].value = bm_client_networking_get_enabled (nmc->client) ? _("enabled") : _("disabled");
				} else
				...
				nmc->get_client (nmc); /* create BMClient */
				bm_client_networking_set_enabled (nmc->client, enable_net);
			}
		}
		else if (matches (*argv, "sleep") == 0) {
			if (next_arg (&argc, &argv) != 0) {
				g_string_printf (nmc->return_text, _("Error: Sleeping status is not exported by BarcodeManager."));
				nmc->return_value = NMC_RESULT_ERROR_UNKNOWN;
			} else {
				if (!strcmp (*argv, "true"))
					sleep_flag = TRUE;
			...
			if (next_arg (&argc, &argv) != 0) {
				...
				if (nmc->required_fields && strcasecmp (nmc->required_fields, "WIFI")) {
					g_string_printf (nmc->return_text, _("Error: '--fields' value '%s' is not valid here; allowed fields: %s"),
					                 nmc->required_fields, NMC_FIELDS_BM_WIFI);
					nmc->return_value = NMC_RESULT_ERROR_USER_INPUT;
					goto end;
				}
				nmc->allowed_fields = nmc_fields_bm_status;
				nmc->print_fields.indices = parse_output_fields (NMC_FIELDS_BM_WIFI, nmc->allowed_fields, NULL);
				nmc->print_fields.flags = multiline_flag | mode_flag | escape_flag | NMC_PF_FLAG_MAIN_HEADER_ADD | NMC_PF_FLAG_FIELD_NAMES;
				...
				if (nmc_is_bm_running (nmc, NULL)) {
					nmc->get_client (nmc); /* create BMClient */
					nmc->allowed_fields[4].value = bm_client_wireless_get_enabled (nmc->client) ? _("enabled") : _("disabled");
				} else
				...
				nmc->get_client (nmc); /* create BMClient */
				bm_client_wireless_set_enabled (nmc->client, enable_wifi);
			}
		}
			...
			if (next_arg (&argc, &argv) != 0) {
				...
				if (nmc->required_fields && strcasecmp (nmc->required_fields, "WWAN")) {
					g_string_printf (nmc->return_text, _("Error: '--fields' value '%s' is not valid here; allowed fields: %s"),
					                 nmc->required_fields, NMC_FIELDS_BM_WWAN);
					nmc->return_value = NMC_RESULT_ERROR_USER_INPUT;
					goto end;
				}
				nmc->allowed_fields = nmc_fields_bm_status;
				nmc->print_fields.indices = parse_output_fields (NMC_FIELDS_BM_WWAN, nmc->allowed_fields, NULL);
				nmc->print_fields.flags = multiline_flag | mode_flag | escape_flag | NMC_PF_FLAG_MAIN_HEADER_ADD | NMC_PF_FLAG_FIELD_NAMES;
				...
				if (nmc_is_bm_running (nmc, NULL)) {
					nmc->get_client (nmc); /* create BMClient */
					nmc->allowed_fields[6].value = bm_client_wwan_get_enabled (nmc->client) ? _("enabled") : _("disabled");
				} else
				...
				nmc->get_client (nmc); /* create BMClient */
				bm_client_wwan_set_enabled (nmc->client, enable_wwan);
			}
		}
