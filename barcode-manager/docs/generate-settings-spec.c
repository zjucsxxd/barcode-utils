...
typedef BMSetting* (*SettingNewFunc) (void);
...
write_one_setting (FILE *f, SettingNewFunc func)
{
	...
	for (iter = props; iter && *iter; iter++) {
		...
		if (!strcmp (key_name, BM_SETTING_NAME)) {
			g_free (default_value);
			default_value = NULL;
			g_object_get (G_OBJECT (s), BM_SETTING_NAME, &default_value, NULL);
		}
...
main (int argc, char *argv[])
{
	...
	if (!bm_utils_init (&error)) {
		fprintf (stderr, "ERR: failed to initialize libbm-util: %s", error->message);
		_exit (2);
	}
	...
	w = fprintf (f, "<html>\n<head>\n");
	w = fprintf (f, "<title>BarcodeManager " PACKAGE_VERSION " Settings Specification</title>");
	w = fprintf (f, "</head>\n<body>\n");
