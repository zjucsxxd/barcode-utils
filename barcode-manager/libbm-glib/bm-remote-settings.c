...
#include "bm-settings-bindings.h"
...
G_DEFINE_TYPE_EXTENDED (BMRemoteSettings, bm_remote_settings, G_TYPE_OBJECT, 0,
                        G_IMPLEMENT_INTERFACE (BM_TYPE_SETTINGS_INTERFACE, settings_interface_init))

#define BM_REMOTE_SETTINGS_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), BM_TYPE_REMOTE_SETTINGS, BMRemoteSettingsPrivate))
...
{
	return g_hash_table_lookup (BM_REMOTE_SETTINGS_GET_PRIVATE (settings)->connections, path);
}

static void
connection_removed_cb (BMRemoteConnection *remote, gpointer user_data)
{
	BMRemoteSettings *self = BM_REMOTE_SETTINGS (user_data);
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
	const char *path;

	path = bm_connection_get_path (BM_CONNECTION (remote));
	g_hash_table_remove (priv->connections, path);
	g_hash_table_remove (priv->pending, path);
}

static void
connection_init_result_cb (BMRemoteConnection *remote,
                           GParamSpec *pspec,
                           gpointer user_data)
{
	BMRemoteSettings *self = BM_REMOTE_SETTINGS (user_data);
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
	guint32 init_result = BM_REMOTE_CONNECTION_INIT_RESULT_UNKNOWN;
	const char *path;
	...
	g_object_get (G_OBJECT (remote),
	              BM_REMOTE_CONNECTION_INIT_RESULT, &init_result,
	              NULL);

	switch (init_result) {
	case BM_REMOTE_CONNECTION_INIT_RESULT_SUCCESS:
		...
		break;
	case BM_REMOTE_CONNECTION_INIT_RESULT_ERROR:
	default:
		break;
	}
...
new_connection_cb (DBusGProxy *proxy, const char *path, gpointer user_data)
{
	BMRemoteSettings *self = BM_REMOTE_SETTINGS (user_data);
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
	BMRemoteConnection *connection;

	connection = bm_remote_connection_new (priv->bus, priv->scope, path);
	if (connection) {
		g_signal_connect (connection, "removed",
		                  G_CALLBACK (connection_removed_cb),
		                  self);

		g_signal_connect (connection, "notify::" BM_REMOTE_CONNECTION_INIT_RESULT,
		                  G_CALLBACK (connection_init_result_cb),
		                  self);
...
fetch_connections (gpointer user_data)
{
	...
	org_freedesktop_BarcodeManagerSettings_list_connections_async (priv->proxy,
...
{
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (settings);
	GSList *list = NULL;
	GHashTableIter iter;
	gpointer value;
	...
	while (g_hash_table_iter_next (&iter, NULL, &value))
		list = g_slist_prepend (list, BM_REMOTE_CONNECTION (value));

	return list;
}

typedef struct {
	BMSettingsInterface *self;
...
add_connection (BMSettingsInterface *settings,
	            ...
	            gpointer user_data)
{
	BMRemoteSettings *self = BM_REMOTE_SETTINGS (settings);
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
	...
	new_settings = bm_connection_to_hash (connection);
	org_freedesktop_BarcodeManagerSettings_add_connection_async (priv->proxy,
	                                                             new_settings,
	                                                             add_connection_done,
	                                                             info);
...
remove_connections (gpointer user_data)
{
	...
	for (list_iter = list; list_iter; list_iter = g_slist_next (list_iter))
		g_signal_emit_by_name (BM_REMOTE_CONNECTION (list_iter->data), "removed");
	g_slist_free (list);
                    ...
                    gpointer user_data)
{
	BMRemoteSettings *self = BM_REMOTE_SETTINGS (user_data);
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
	const char *sname = BM_DBUS_SERVICE_USER_SETTINGS;

	if (priv->scope == BM_CONNECTION_SCOPE_SYSTEM)
		sname = BM_DBUS_SERVICE_SYSTEM_SETTINGS;
	...
	if (!strcmp (name, sname)) {
		...
		g_object_notify (G_OBJECT (self), BM_REMOTE_SETTINGS_SERVICE_RUNNING);
	}
}
 ...
 **/
BMRemoteSettings *
bm_remote_settings_new (DBusGConnection *bus, BMConnectionScope scope)
{
	g_return_val_if_fail (bus != NULL, NULL);
	g_return_val_if_fail (scope != BM_CONNECTION_SCOPE_UNKNOWN, NULL);

	return (BMRemoteSettings *) g_object_new (BM_TYPE_REMOTE_SETTINGS,
	                                          BM_REMOTE_SETTINGS_BUS, bus,
	                                          BM_REMOTE_SETTINGS_SCOPE, scope,
	                                          NULL);
}

static void
bm_remote_settings_init (BMRemoteSettings *self)
{
	BMRemoteSettingsPrivate *priv = BM_REMOTE_SETTINGS_GET_PRIVATE (self);
...
dispose (GObject *object)
{
	...
	G_OBJECT_CLASS (bm_remote_settings_parent_class)->dispose (object);
}
...
static void
bm_remote_settings_class_init (BMRemoteSettingsClass *class)
{
	GObjectClass *object_class = G_OBJECT_CLASS (class);

	g_type_class_add_private (class, sizeof (BMRemoteSettingsPrivate));
	...
	g_object_class_install_property
		(object_class, PROP_BUS,
		 g_param_spec_boxed (BM_REMOTE_SETTINGS_BUS,
		                     "DBusGConnection",
		                     "DBusGConnection",
	...
	g_object_class_install_property
		(object_class, PROP_SCOPE,
		 g_param_spec_uint (BM_REMOTE_SETTINGS_SCOPE,
		                    "Scope",
		                    "BMConnection scope",
	...
	g_object_class_install_property
		(object_class, PROP_SERVICE_RUNNING,
		 g_param_spec_boolean (BM_REMOTE_SETTINGS_SERVICE_RUNNING,
		                       "Service running",
		                       "Is service running",
		                       FALSE,
		                       G_PARAM_READABLE));
