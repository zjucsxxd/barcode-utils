...
static void
default_call_func (BMAuthChain *chain,
...
gpointer
bm_auth_chain_get_data (BMAuthChain *self, const char *tag)
{
	ChainData *tmp;
...
void
bm_auth_chain_set_data (BMAuthChain *self,
                        const char *tag,
                        gpointer data,
...
static void
bm_auth_chain_check_done (BMAuthChain *self)
{
	g_return_if_fail (self != NULL);
...
void
bm_auth_chain_unref (BMAuthChain *self)
{
	GSList *iter;
	...
	memset (self, 0, sizeof (BMAuthChain));
	g_free (self);
}
