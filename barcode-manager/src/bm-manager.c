...
#include "bm-device-cdma.h"
#include "bm-device-gsm.h"
#include "bm-system.h"
...
#define BM_AUTOIP_DBUS_SERVICE "org.freedesktop.bm_avahi_autoipd"
#define BM_AUTOIP_DBUS_IFACE   "org.freedesktop.bm_avahi_autoipd"
...
static void add_device (NMManager *self, BMDevice *device);
...
static BMDevice *find_device_by_iface (NMManager *self, const gchar *iface);
...
static BMDevice *bm_manager_get_device_by_udi (NMManager *manager, const char *udi);
...
#define BM_MANAGER_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), BM_TYPE_MANAGER, NMManagerPrivate))

G_DEFINE_TYPE_EXTENDED (NMManager, bm_manager, G_TYPE_OBJECT, 0,
						G_IMPLEMENT_INTERFACE (BM_TYPE_HOSTNAME_PROVIDER,
...
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
...
vpn_manager_connection_deactivated_cb (NMVPNManager *manager,
                                       ...
                                       gpointer user_data)
{
	g_object_notify (G_OBJECT (user_data), BM_MANAGER_ACTIVE_CONNECTIONS);
}
...
modem_added (NMModemManager *modem_manager,
			 ...
			 gpointer user_data)
{
	NMManager *self = BM_MANAGER (user_data);
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	BMDevice *replace_device, *device = NULL;
	const char *ip_iface;
	GSList *iter;

	ip_iface = bm_modem_get_iface (modem);

	replace_device = find_device_by_iface (BM_MANAGER (user_data), ip_iface);
	if (replace_device) {
		priv->devices = remove_one_device (BM_MANAGER (user_data),
		                                   priv->devices,
		                                   replace_device,
		                                   FALSE);
	}
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		if (BM_IS_DEVICE_BT (iter->data)) {
			if (bm_device_bt_modem_added (BM_DEVICE_BT (iter->data), modem, driver))
				return;
		}
	}
	...
	if (driver && !strcmp (driver, "bluetooth")) {
		bm_log_info (LOGD_MB, "ignoring modem '%s' (no associated Bluetooth device)", ip_iface);
		return;
	}

	/* Otherwise make a new top-level BMDevice for it */
	if (BM_IS_MODEM_GSM (modem))
		device = bm_device_gsm_new (BM_MODEM_GSM (modem), driver);
	else if (BM_IS_MODEM_CDMA (modem))
		device = bm_device_cdma_new (BM_MODEM_CDMA (modem), driver);
	else
		bm_log_info (LOGD_MB, "unhandled modem '%s'", ip_iface);
...
static void
bm_manager_update_state (NMManager *manager)
{
...
static void
update_active_connection_timestamp (NMManager *manager, BMDevice *device)
{
	NMActRequest *req;
	BMConnection *connection;
	...
	*ts_ptr = timestamp;
	...
	if (!g_key_file_load_from_file (timestamps_file, BM_SYSCONFIG_SETTINGS_TIMESTAMPS_FILE, G_KEY_FILE_KEEP_COMMENTS, &error)) {
		if (!(error->domain == G_FILE_ERROR && error->code == G_FILE_ERROR_NOENT))
			bm_log_warn (LOGD_SYS_SET, "error parsing timestamps file '%s': %s", BM_SYSCONFIG_SETTINGS_TIMESTAMPS_FILE, error->message);
		g_clear_error (&error);
	}
	...
	if (error) {
		bm_log_warn (LOGD_SYS_SET, "error saving timestamp: %s", error->message);
		g_error_free (error);
	}
...
manager_device_state_changed (BMDevice *device,
                              ...
                              gpointer user_data)
{
	NMManager *manager = BM_MANAGER (user_data);
...
remove_one_device (NMManager *manager,
                   ...
                   gboolean quitting)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
			   ...
			   gpointer user_data)
{
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		if (BM_IS_DEVICE_BT (iter->data)) {
			if (bm_device_bt_modem_removed (BM_DEVICE_BT (iter->data), modem))
				return;
		}
	}

	/* Otherwise remove the standalone modem */
	found = bm_manager_get_device_by_udi (self, bm_modem_get_path (modem));
	if (found)
                   ...
                   gpointer user_data)
{
	...
	if (!event || !iface) {
		bm_log_warn (LOGD_AUTOIP4, "incomplete message received from avahi-autoipd");
		return;
	}
	...
	if (   (strcmp (event, "BIND") != 0)
	    ...
	    && (strcmp (event, "STOP") != 0)) {
		bm_log_warn (LOGD_AUTOIP4, "unknown event '%s' received from avahi-autoipd", event);
		return;
	}

	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		BMDevice *candidate = BM_DEVICE (iter->data);

		if (!strcmp (bm_device_get_iface (candidate), iface)) {
			bm_device_handle_autoip4_event (candidate, event, address);
			handled = TRUE;
			break;
		}
	}
	...
	if (!handled)
		bm_log_warn (LOGD_AUTOIP4, "(%s): unhandled avahi-autoipd event", iface);
}

static const char *
hostname_provider_get_hostname (NMHostnameProvider *provider)
{
	return BM_MANAGER_GET_PRIVATE (provider)->hostname;
}
...
BMState
bm_manager_get_state (NMManager *manager)
{
...
static void
bm_manager_pending_activation_remove (NMManager *self,
                        ...
                        gpointer user_data)
{
	 ...
	 * check for 'yes' here.
	 */
	result = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, BM_AUTH_PERMISSION_USE_USER_CONNECTIONS));
	if (result != BM_AUTH_CALL_RESULT_YES) {
                       ...
                       gpointer user_data)
{
	 ...
	 * check for 'yes' here.
	 */
	result = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, BM_AUTH_PERMISSION_NETWORK_CONTROL));
	if (result != BM_AUTH_CALL_RESULT_YES) {
		error = g_error_new_literal (BM_MANAGER_ERROR,
		                             BM_MANAGER_ERROR_PERMISSION_DENIED,
	...
	if (pending->scope == BM_CONNECTION_SCOPE_SYSTEM) {
		/* System connection and the user is authorized for that if they have
		 * the network-control permission.
		 */
		pending->callback (pending, NULL);
	} else {
		g_assert (pending->scope == BM_CONNECTION_SCOPE_USER);

		/* User connection, check the 'use-user-connections' permission */
		pending->chain = bm_auth_chain_new (pending->authority,
			                                pending->context,
			                                NULL,
			                                pending_auth_user_done,
			                                pending);
		bm_auth_chain_add_call (pending->chain,
			                    BM_AUTH_PERMISSION_USE_USER_CONNECTIONS,
			                    TRUE);
	}
                       ...
                       const char **out_error_desc)
{
	...
	*out_sender_uid = G_MAXULONG;
	...
	/* Get the UID */
	if (!bm_auth_get_caller_uid (context, dbus_mgr, out_sender_uid, out_error_desc))
		return FALSE;
	...
	if (   scope == BM_CONNECTION_SCOPE_USER
	    && !bm_auth_uid_authorized (*out_sender_uid,
	                                dbus_mgr,
	                                user_proxy,
	                                out_error_desc))
		return FALSE;
                                     ...
                                     DBusGProxy *user_proxy)
{
	...
	bm_auth_chain_add_call (pending->chain,
	                        BM_AUTH_PERMISSION_NETWORK_CONTROL,
	                        TRUE);
}
                            ...
                            const char *ac_path)
{
	...
	if (pending->chain)
		bm_auth_chain_unref (pending->chain);
...
static GPtrArray *
get_active_connections (NMManager *manager, BMConnection *filter)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
	NMVPNManager *vpn_manager;
	GPtrArray *active;
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		req = bm_device_get_act_request (BM_DEVICE (iter->data));
		if (!req)
			continue;

		if (!filter || (bm_act_request_get_connection (req) == filter)) {
			path = bm_act_request_get_active_connection_path (req);
	...
	bm_vpn_manager_add_active_connections (vpn_manager, filter, active);
...
user_proxy_cleanup (NMManager *self, gboolean resync_bt)
{
	...
	priv->user_net_perm = BM_AUTH_CALL_RESULT_UNKNOWN;
	priv->user_con_perm = BM_AUTH_CALL_RESULT_UNKNOWN;
...
static void
system_connection_updated_cb (BMSettingsConnectionInterface *connection,
                              gpointer unused,
                              NMManager *manager)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
	const char *path;
	BMSettingsConnectionInterface *existing;
	GError *error = NULL;
	...
	if (existing != connection) {
		bm_log_warn (LOGD_SYS_SET, "existing connection didn't matched updated.");
		return;
	}

	if (!bm_connection_verify (BM_CONNECTION (existing), &error)) {
		/* Updated connection invalid, remove existing connection */
		bm_log_warn (LOGD_SYS_SET, "invalid connection: '%s' / '%s' invalid: %d",
		             g_type_name (bm_connection_lookup_setting_type_by_quark (error->domain)),
		             error->message, error->code);
		g_error_free (error);
		remove_connection (manager, BM_CONNECTION (existing), priv->system_connections);
		return;
	}

	g_signal_emit (manager, signals[CONNECTION_UPDATED], 0,
	               existing, BM_CONNECTION_SCOPE_SYSTEM);
                                ...
                                BMSettingsConnectionInterface *connection)
{
	...
	g_signal_connect (connection, BM_SETTINGS_CONNECTION_INTERFACE_UPDATED,
	                  G_CALLBACK (system_connection_updated_cb), manager);
	g_signal_connect (connection, BM_SETTINGS_CONNECTION_INTERFACE_REMOVED,
	                  G_CALLBACK (system_connection_removed_cb), manager);

	path = bm_connection_get_path (BM_CONNECTION (connection));
	g_hash_table_insert (priv->system_connections, g_strdup (path),
	                     g_object_ref (connection));
	g_signal_emit (manager, signals[CONNECTION_ADDED], 0, connection, BM_CONNECTION_SCOPE_SYSTEM);
}
...
system_query_connections (NMManager *manager)
{
	...
	system_connections = bm_settings_interface_list_connections (BM_SETTINGS_INTERFACE (priv->sys_settings));
	for (iter = system_connections; iter; iter = g_slist_next (iter))
		system_internal_new_connection (manager, BM_SETTINGS_CONNECTION_INTERFACE (iter->data));
                                     ...
                                     gpointer user_data)
{
	...
	unmanaged_specs = bm_sysconfig_settings_get_unmanaged_specs (sys_settings);
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		BMDevice *device = BM_DEVICE (iter->data);
		gboolean managed;

		managed = !bm_device_interface_spec_match_list (BM_DEVICE_INTERFACE (device), unmanaged_specs);
		bm_device_set_managed (device,
		                       managed,
		                       managed ? BM_DEVICE_STATE_REASON_NOW_MANAGED :
		                                   BM_DEVICE_STATE_REASON_NOW_UNMANAGED);
	}
}
                            ...
                            gpointer user_data)
{
	...
	hostname = bm_sysconfig_settings_get_hostname (sys_settings);
	...
	g_object_notify (G_OBJECT (manager), BM_MANAGER_HOSTNAME);
...
static BMDevice *
bm_manager_get_device_by_udi (NMManager *manager, const char *udi)
{
	GSList *iter;

	for (iter = BM_MANAGER_GET_PRIVATE (manager)->devices; iter; iter = iter->next) {
		if (!strcmp (bm_device_get_udi (BM_DEVICE (iter->data)), udi))
...
static BMDevice *
bm_manager_get_device_by_path (NMManager *manager, const char *path)
{
	GSList *iter;
...
manager_update_radio_enabled (NMManager *self, RadioState *rstate)
{
	...
	for (iter = priv->devices; iter; iter = iter->next) {
		...
		g_object_get (G_OBJECT (iter->data), BM_DEVICE_INTERFACE_RFKILL_TYPE, &devtype, NULL);
		if (devtype == rstate->rtype) {
			bm_log_dbg (LOGD_RFKILL, "(%s): setting radio %s",
			            bm_device_get_iface (BM_DEVICE (iter->data)),
			            enabled ? "enabled" : "disabled");
			bm_device_interface_set_enabled (BM_DEVICE_INTERFACE (iter->data), enabled);
		}
	}
}

static void
manager_hidden_ap_found (BMDeviceInterface *device,
                         NMAccessPoint *ap,
                         gpointer user_data)
{
	NMManager *manager = BM_MANAGER (user_data);
	...
	ap_ssid = bm_ap_get_ssid (ap);
	if (ap_ssid && ap_ssid->len)
		return;

	ap_addr = bm_ap_get_address (ap);
	g_assert (ap_addr);
	...
	for (iter = connections; iter && !done; iter = g_slist_next (iter)) {
		...
		BMSettingWireless *s_wireless;
		const GByteArray *ssid;
		guint32 num_bssids;
		guint32 i;

		s_wireless = (BMSettingWireless *) bm_connection_get_setting (connection, BM_TYPE_SETTING_WIRELESS);
		if (!s_wireless)
			goto next;

		num_bssids = bm_setting_wireless_get_num_seen_bssids (s_wireless);
		if (num_bssids < 1)
			goto next;

		ssid = bm_setting_wireless_get_ssid (s_wireless);
		g_assert (ssid);

		for (i = 0; i < num_bssids; i++) {
			const char *seen_bssid = bm_setting_wireless_get_seen_bssid (s_wireless, i);
...
static RfKillState
bm_manager_get_ipw_rfkill_state (NMManager *self)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	GSList *iter;
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		if (BM_IS_DEVICE_WIFI (candidate)) {
			candidate_state = bm_device_wifi_get_ipw_rfkill_state (BM_DEVICE_WIFI (candidate));
...
static RfKillState
bm_manager_get_modem_enabled_state (NMManager *self)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	GSList *iter;
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		g_object_get (G_OBJECT (candidate), BM_DEVICE_INTERFACE_RFKILL_TYPE, &devtype, NULL);
		if (devtype == RFKILL_TYPE_WWAN) {
			if (!bm_device_interface_get_enabled (BM_DEVICE_INTERFACE (candidate)))
                                ...
                                RfKillType rtype)
{
	...
	udev_state = bm_udev_manager_get_rfkill_state (priv->udev_mgr, rtype);
	...
	if (rstate->desc) {
		bm_log_dbg (LOGD_RFKILL, "%s hw-enabled %d sw-enabled %d",
	...
	if (old_rfkilled != new_rfkilled) {
		bm_log_info (LOGD_RFKILL, "%s now %s by radio killswitch",
		             rstate->desc,
...
static void
bm_manager_rfkill_update (NMManager *self, RfKillType rtype)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	guint i;
...
static void
manager_ipw_rfkill_state_changed (BMDeviceWifi *device,
                                  GParamSpec *pspec,
                                  gpointer user_data)
{
	bm_manager_rfkill_update (BM_MANAGER (user_data), RFKILL_TYPE_WLAN);
}

static void
manager_modem_enabled_changed (NMModem *device, gpointer user_data)
{
	bm_manager_rfkill_update (BM_MANAGER (user_data), RFKILL_TYPE_WWAN);
}
...
{
	if (auth_error) {
		bm_log_dbg (LOGD_CORE, "%s request failed: %s", detail, auth_error->message);
		return g_error_new (BM_MANAGER_ERROR,
		                    BM_MANAGER_ERROR_PERMISSION_DENIED,
		                    "%s request failed: %s",
                              ...
                              gpointer user_data)
{
	...
	if (!ret_error) {
		...
		device = bm_auth_chain_get_data (chain, "device");
		if (bm_device_interface_disconnect (BM_DEVICE_INTERFACE (device), &ret_error))
                             ...
                             gpointer user_data)
{
	...
	device = bm_auth_chain_get_data (chain, "device");
	scope = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, "scope"));
	if (scope == BM_CONNECTION_SCOPE_USER) {
		 ...
		 */
		user_chain = bm_auth_chain_new (priv->authority, context, NULL, disconnect_user_auth_done_cb, self);
		g_assert (user_chain);
		priv->auth_chains = g_slist_append (priv->auth_chains, user_chain);

		bm_auth_chain_set_data (user_chain, "device", g_object_ref (device), g_object_unref);
		bm_auth_chain_set_data (user_chain, "scope", GUINT_TO_POINTER (scope), NULL);
		bm_auth_chain_add_call (user_chain, BM_AUTH_PERMISSION_USE_USER_CONNECTIONS, TRUE);
	} else {
		if (!bm_device_interface_disconnect (BM_DEVICE_INTERFACE (device), &ret_error)) {
			dbus_g_method_return_error (context, ret_error);
			g_clear_error (&ret_error);
...
static void
manager_device_disconnect_request (BMDevice *device,
                                   DBusGMethodInvocation *context,
                                   NMManager *self)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	...
	if (!req) {
		error = g_error_new_literal (BM_MANAGER_ERROR,
		                             BM_MANAGER_ERROR_UNKNOWN_CONNECTION,
	 ...
	 */
	scope = bm_connection_get_scope (connection);
	...
	/* Yay for root */
	if (0 == sender_uid) {
		if (!bm_device_interface_disconnect (BM_DEVICE_INTERFACE (device), &error)) {
			dbus_g_method_return_error (context, error);
			g_clear_error (&error);
		} else
			dbus_g_method_return (context);
		...
		chain = bm_auth_chain_new (priv->authority, context, NULL, disconnect_net_auth_done_cb, self);
		g_assert (chain);
		priv->auth_chains = g_slist_append (priv->auth_chains, chain);

		bm_auth_chain_set_data (chain, "device", g_object_ref (device), g_object_unref);
		bm_auth_chain_set_data (chain, "scope", GUINT_TO_POINTER (scope), NULL);
		bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_NETWORK_CONTROL, TRUE);
	}
}

static void
add_device (NMManager *self, BMDevice *device)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	...
	iface = bm_device_get_ip_iface (device);
	g_assert (iface);
	...
	if (BM_IS_DEVICE_WIFI (device)) {
		 ...
		 */
		g_signal_connect (device, "notify::" BM_DEVICE_WIFI_IPW_RFKILL_STATE,
		 ...
		 */
		bm_manager_rfkill_update (self, RFKILL_TYPE_WLAN);
		enabled = radio_enabled_for_type (self, RFKILL_TYPE_WLAN);
		bm_device_interface_set_enabled (BM_DEVICE_INTERFACE (device), enabled);
	} else if (BM_IS_DEVICE_MODEM (device)) {
		g_signal_connect (device, BM_DEVICE_MODEM_ENABLE_CHANGED,
		                  G_CALLBACK (manager_modem_enabled_changed),
		                  self);

		bm_manager_rfkill_update (self, RFKILL_TYPE_WWAN);
		 ...
		 * touch it here.
		bm_device_interface_set_enabled (BM_DEVICE_INTERFACE (device),
	...
	if (!driver)
		driver = "unknown";
	bm_log_info (LOGD_HW, "(%s): new %s device (driver: '%s' ifindex: %d)",
	             iface, type_desc, driver, bm_device_get_ifindex (device));
	...
	if (bm_device_interface_can_assume_connections (BM_DEVICE_INTERFACE (device))) {
		...
		existing = bm_device_interface_connection_match_config (BM_DEVICE_INTERFACE (device),
		...
		if (existing) {
			...
			s_con = (BMSettingConnection *) bm_connection_get_setting (existing, BM_TYPE_SETTING_CONNECTION);
			bm_log_dbg (LOGD_DEVICE, "(%s): found existing device connection '%s'",
	...
	unmanaged_specs = bm_sysconfig_settings_get_unmanaged_specs (priv->sys_settings);
	if (   !manager_sleeping (self)
	    && !bm_device_interface_spec_match_list (BM_DEVICE_INTERFACE (device), unmanaged_specs)) {
		bm_device_set_managed (device,
		                       TRUE,
		                       existing ? BM_DEVICE_STATE_REASON_CONNECTION_ASSUMED :
		                                  BM_DEVICE_STATE_REASON_NOW_MANAGED);
		managed = TRUE;
	}
	...
	if (existing && managed && bm_device_is_available (device)) {
		const char *ac_path;
		GError *error = NULL;

		bm_log_dbg (LOGD_DEVICE, "(%s): will attempt to assume existing connection",
		            bm_device_get_iface (device));

		ac_path = internal_activate_device (self, device, existing, NULL, FALSE, TRUE, &error);
		if (ac_path)
			g_object_notify (G_OBJECT (self), BM_MANAGER_ACTIVE_CONNECTIONS);
		else {
			bm_log_warn (LOGD_DEVICE, "assumed connection (%d) %s failed to activate: (%d) %s",
			             bm_connection_get_scope (existing),
			             bm_connection_get_path (existing),
			             error ? error->code : -1,
			             error && error->message ? error->message : "(unknown)");
			g_error_free (error);
		}
...
bluez_manager_resync_devices (NMManager *self)
{
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		if (BM_IS_DEVICE_BT (candidate)) {
			uuids = bm_device_bt_get_capabilities (BM_DEVICE_BT (candidate));
			bdaddr = bm_device_bt_get_hw_address (BM_DEVICE_BT (candidate));
	...
	if (g_slist_length (gone)) {
		...
		while (g_slist_length (gone))
			gone = remove_one_device (self, gone, BM_DEVICE (gone->data), FALSE);
	} else {
		g_slist_free (keep);
		g_slist_free (gone);
	}
	...
	bm_bluez_manager_query_devices (priv->bluez_mgr);
}
...
{
	BMDevice *device;
	gboolean has_dun = (capabilities & BM_BT_CAPABILITY_DUN);
	gboolean has_nap = (capabilities & BM_BT_CAPABILITY_NAP);

	g_return_if_fail (bdaddr != NULL);
	g_return_if_fail (name != NULL);
	g_return_if_fail (object_path != NULL);
	g_return_if_fail (capabilities != BM_BT_CAPABILITY_NONE);

	/* Make sure the device is not already in the device list */
	if (bm_manager_get_device_by_udi (manager, object_path))
		return;
	...
	device = bm_device_bt_new (object_path, bdaddr, name, capabilities, FALSE);
	if (device) {
		bm_log_info (LOGD_HW, "BT device %s (%s) added (%s%s%s)",
		             name,
		             bdaddr,
		             has_dun ? "DUN" : "",
		             has_dun && has_nap ? " " : "",
                                 ...
                                 gpointer user_data)
{
	...
	bm_log_info (LOGD_HW, "BT device %s removed", bdaddr);

	for (iter = priv->devices; iter; iter = iter->next) {
		BMDevice *device = BM_DEVICE (iter->data);

		if (!strcmp (bm_device_get_udi (device), object_path)) {
...
find_device_by_iface (NMManager *self, const gchar *iface)
{
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		BMDevice *device = BM_DEVICE (iter->data);
		const gchar *d_iface = bm_device_get_ip_iface (device);
...
find_device_by_ifindex (NMManager *self, guint32 ifindex)
{
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		if (ifindex == bm_device_get_ifindex (candidate))
			return candidate;
	}
	return NULL;
}
...
udev_manager_rfkill_changed_cb (NMUdevManager *udev_mgr,
                                ...
                                gpointer user_data)
{
	bm_manager_rfkill_update (BM_MANAGER (user_data), rtype);
}

GSList *
bm_manager_get_devices (NMManager *manager)
{
	g_return_val_if_fail (BM_IS_MANAGER (manager), NULL);

	return BM_MANAGER_GET_PRIVATE (manager)->devices;
}
...
static NMActRequest *
bm_manager_get_act_request_by_path (NMManager *manager,
                                    const char *path,
                                    BMDevice **device)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		ac_path = bm_act_request_get_active_connection_path (req);
		if (!strcmp (path, ac_path)) {
			*device = BM_DEVICE (iter->data);
			return req;
		}
	}
...
free_get_secrets_info (gpointer data)
{
	...
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (info->manager);
...
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (user_data);
	GSList *iter;
                     ...
                     gpointer user_data)
{
	...
	if (dbus_g_proxy_end_call (proxy, call, &error,
	                           DBUS_TYPE_G_MAP_OF_MAP_OF_VARIANT, &settings,
	                           G_TYPE_INVALID)) {
		bm_secrets_provider_interface_get_secrets_result (info->provider,
		                                                  info->setting_name,
		                                                  info->caller,
		                                                  settings,
		                                                  NULL);
                  ...
                  const char *hint2)
{
	...
	if (!info->proxy) {
		bm_log_warn (LOGD_USER_SET, "could not create user connection secrets proxy");
		g_free (info);
		return NULL;
	}
...
static void
system_get_secrets_reply_cb (BMSettingsConnectionInterface *connection,
                             GHashTable *secrets,
                             GError *error,
                             gpointer user_data)
{
...
system_get_secrets_idle_cb (gpointer user_data)
{
	...
	connection = bm_settings_interface_get_connection_by_path (BM_SETTINGS_INTERFACE (priv->sys_settings), 
	...
	hints[0] = info->hint1;
	hints[1] = info->hint2;
	bm_settings_connection_interface_get_secrets (connection,
	                                              info->setting_name,
	                                              hints,
	                                              info->request_new,
                    ...
                    const char *hint2)
{
	...
	info->connection_path = g_strdup (bm_connection_get_path (connection));
                          ...
                          GError **error)
{
	...
	dev_iface = BM_DEVICE_INTERFACE (device);

	/* Ensure the requested connection is compatible with the device */
	if (!bm_device_interface_check_connection_compatible (dev_iface, connection, error))
		return NULL;
	...
	req = bm_act_request_new (connection, specific_object, user_requested, assumed, (gpointer) device);
	...
	success = bm_device_interface_activate (dev_iface, req, error);
	g_object_unref (req);

	return success ? bm_act_request_get_active_connection_path (req) : NULL;
}
...
wait_for_connection_expired (gpointer data)
{
	...
	bm_log_warn (LOGD_CORE, "connection %s (scope %d) failed to activate (timeout)",
	             pending->connection_path, pending->scope);

	bm_manager_pending_activation_remove (pending->manager, pending);
...
const char *
bm_manager_activate_connection (NMManager *manager,
                                BMConnection *connection,
                                const char *specific_object,
                                const char *device_path,
                                gboolean user_requested,
                                GError **error)
{
	...
	if (!strcmp (bm_setting_connection_get_connection_type (s_con), BM_SETTING_VPN_SETTING_NAME)) {
		...
		if (specific_object) {
			...
			req = bm_manager_get_act_request_by_path (manager, specific_object, &device);
			if (!req) {
				g_set_error (error,
				             BM_MANAGER_ERROR, BM_MANAGER_ERROR_CONNECTION_NOT_ACTIVE,
			...
			for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
				...
				candidate_req = bm_device_get_act_request (candidate);
				if (candidate_req && bm_act_request_get_default (candidate_req)) {
					device = candidate;
					req = candidate_req;
					break;
				}
		...
		if (!device || !req) {
			g_set_error (error,
			             BM_MANAGER_ERROR, BM_MANAGER_ERROR_UNKNOWN_DEVICE,
			             "%s", "Could not find source connection, or the source connection had no active device.");
			return NULL;
		}

		vpn_manager = bm_vpn_manager_get ();
		vpn_connection = bm_vpn_manager_activate_connection (vpn_manager,
		                                                     connection,
		                                                     req,
		                                                     device,
		                                                     error);
		if (vpn_connection) {
			...
			path = bm_vpn_connection_get_active_connection_path (vpn_connection);
		}
		g_object_unref (vpn_manager);
	} else {
		BMDeviceState state;
		...
		device = bm_manager_get_device_by_path (manager, device_path);
		if (!device) {
			g_set_error (error,
			             BM_MANAGER_ERROR, BM_MANAGER_ERROR_UNKNOWN_DEVICE,
			             "%s", "Device not found");
		...
		if (state < BM_DEVICE_STATE_DISCONNECTED) {
			g_set_error (error,
			             BM_MANAGER_ERROR, BM_MANAGER_ERROR_UNMANAGED_DEVICE,
			             "%s", "Device not managed by BarcodeManager or unavailable");
			return NULL;
		}
...
static PendingActivation *
bm_manager_pending_activation_find (NMManager *self,
                                    const char *path,
                                    BMConnectionScope scope)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
...
check_pending_ready (NMManager *self, PendingActivation *pending)
{
	...
	bm_manager_pending_activation_remove (self, pending);

	connection = bm_manager_get_connection_by_object_path (self,
	                                                       pending->scope,
	...
	path = bm_manager_activate_connection (self,
	                                       connection,
	                                       ...
	                                       &error);
	if (!path) {
		bm_log_warn (LOGD_CORE, "connection (%d) %s failed to activate: (%d) %s",
		             pending->scope, pending->connection_path, error->code, error->message);
	} else
		g_object_notify (G_OBJECT (pending->manager), BM_MANAGER_ACTIVE_CONNECTIONS);
                                  ...
                                  BMConnectionScope scope)
{
	...
	pending = bm_manager_pending_activation_find (self,
	                                              bm_connection_get_path (connection),
	                                              scope);
	if (pending) {
...
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	BMConnectionScope scope = BM_CONNECTION_SCOPE_UNKNOWN;
	PendingActivation *pending;
	GError *error = NULL;

	if (!strcmp (service_name, BM_DBUS_SERVICE_USER_SETTINGS))
		scope = BM_CONNECTION_SCOPE_USER;
	else if (!strcmp (service_name, BM_DBUS_SERVICE_SYSTEM_SETTINGS))
		scope = BM_CONNECTION_SCOPE_SYSTEM;
	else {
		error = g_error_new_literal (BM_MANAGER_ERROR,
		                             BM_MANAGER_ERROR_INVALID_SERVICE,
	...
	if (bm_manager_get_connection_by_object_path (self, scope, connection_path))
		pending->have_connection = TRUE;
...
gboolean
bm_manager_deactivate_connection (NMManager *manager,
                                  const char *connection_path,
                                  BMDeviceStateReason reason,
                                  GError **error)
{
	...
	NMVPNConnectionStateReason vpn_reason = BM_VPN_CONNECTION_STATE_REASON_USER_DISCONNECTED;
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		if (!strcmp (connection_path, bm_act_request_get_active_connection_path (req))) {
			bm_device_state_changed (device,
			                         BM_DEVICE_STATE_DISCONNECTED,
			                         reason);
			success = TRUE;
	...
	vpn_manager = bm_vpn_manager_get ();
	if (reason == BM_DEVICE_STATE_REASON_CONNECTION_REMOVED)
		vpn_reason = BM_VPN_CONNECTION_STATE_REASON_CONNECTION_REMOVED;
	if (bm_vpn_manager_deactivate_connection (vpn_manager, connection_path, vpn_reason)) {
		success = TRUE;
	} else {
		g_set_error (error,
		             BM_MANAGER_ERROR, BM_MANAGER_ERROR_CONNECTION_NOT_ACTIVE,
                              ...
                              gpointer user_data)
{
	...
	if (!ret_error) {
		/* Everything authorized, deactivate the connection */
		if (bm_manager_deactivate_connection (self,
		                                      bm_auth_chain_get_data (chain, "path"),
                             ...
                             gpointer user_data)
{
	...
	/* If it's a system connection, we're done */
	active_path = bm_auth_chain_get_data (chain, "path");
	scope = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, "scope"));
	if (scope == BM_CONNECTION_SCOPE_USER) {
		NMAuthChain *user_chain;
		 ...
		 * authorized to manipulate user connections.
		 */
		user_chain = bm_auth_chain_new (priv->authority, context, NULL, deactivate_user_auth_done_cb, self);
		g_assert (user_chain);
		priv->auth_chains = g_slist_append (priv->auth_chains, user_chain);

		bm_auth_chain_set_data (user_chain, "path", g_strdup (active_path), g_free);
		bm_auth_chain_set_data (user_chain, "scope", GUINT_TO_POINTER (scope), NULL);
		bm_auth_chain_add_call (user_chain, BM_AUTH_PERMISSION_USE_USER_CONNECTIONS, TRUE);
	} else {
		if (!bm_manager_deactivate_connection (self,
                                    ...
                                    DBusGMethodInvocation *context)
{
	...
	for (iter = priv->devices; iter; iter = g_slist_next (iter)) {
		...
		req = bm_device_get_act_request (BM_DEVICE (iter->data));
		if (req)
			req_path = bm_act_request_get_active_connection_path (req);
	...
	/* Maybe it's a VPN */
	if (!connection)
		connection = bm_vpn_manager_get_connection_for_active (priv->vpn_manager, active_path);
	...
	/* Otherwise validate the user request */
	chain = bm_auth_chain_new (priv->authority, context, NULL, deactivate_net_auth_done_cb, self);
	g_assert (chain);
	priv->auth_chains = g_slist_append (priv->auth_chains, chain);

	bm_auth_chain_set_data (chain, "path", g_strdup (active_path), g_free);
	bm_auth_chain_set_data (chain, "scope", GUINT_TO_POINTER (scope), NULL);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_NETWORK_CONTROL, TRUE);
}
...
do_sleep_wake (NMManager *self)
{
	...
	if (manager_sleeping (self)) {
		 ...
		 */
		for (iter = priv->devices; iter; iter = iter->next)
			bm_device_set_managed (BM_DEVICE (iter->data), FALSE, BM_DEVICE_STATE_REASON_SLEEPING);
		 ...
		 */
		bm_manager_rfkill_update (self, RFKILL_TYPE_UNKNOWN);
		...
		for (iter = priv->devices; iter; iter = iter->next) {
			...
			for (i = 0; i < RFKILL_TYPE_MAX; i++) {
				...
				if (rstate->desc) {
					bm_log_dbg (LOGD_RFKILL, "%s %s devices (hw_enabled %d, sw_enabled %d, user_enabled %d)",
					            enabled ? "enabling" : "disabling",
					            rstate->desc, rstate->hw_enabled, rstate->sw_enabled, rstate->user_enabled);
				}

				g_object_get (G_OBJECT (device), BM_DEVICE_INTERFACE_RFKILL_TYPE, &devtype, NULL);
				if (devtype == rstate->rtype)
					bm_device_interface_set_enabled (BM_DEVICE_INTERFACE (device), enabled);
			}

			bm_device_clear_autoconnect_inhibit (device);
			if (bm_device_interface_spec_match_list (BM_DEVICE_INTERFACE (device), unmanaged_specs))
				bm_device_set_managed (device, FALSE, BM_DEVICE_STATE_REASON_NOW_UNMANAGED);
			else
				bm_device_set_managed (device, TRUE, BM_DEVICE_STATE_REASON_NOW_MANAGED);
		}
...
_internal_sleep (NMManager *self, gboolean do_sleep)
{
	...
	g_object_notify (G_OBJECT (self), BM_MANAGER_SLEEPING);
}
                    ...
                    gpointer user_data)
{
	...
	result = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, BM_AUTH_PERMISSION_SLEEP_WAKE));
	if (error) {
		bm_log_dbg (LOGD_SUSPEND, "Sleep/wake request failed: %s", error->message);
		...
		/* Auth success */
		do_sleep = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, "sleep"));
                    ...
                    DBusGMethodInvocation *context)
{
	...
	chain = bm_auth_chain_new (priv->authority, context, NULL, sleep_auth_done_cb, self);
	g_assert (chain);
	priv->auth_chains = g_slist_append (priv->auth_chains, chain);

	bm_auth_chain_set_data (chain, "sleep", GUINT_TO_POINTER (do_sleep), NULL);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_SLEEP_WAKE, TRUE);
#endif
}
...
{
	bm_log_dbg (LOGD_SUSPEND, "Received UPower sleeping signal");
	_internal_sleep (BM_MANAGER (user_data), TRUE);
}

static void
upower_resuming_cb (DBusGProxy *proxy, gpointer user_data)
{
	bm_log_dbg (LOGD_SUSPEND, "Received UPower resuming signal");
	_internal_sleep (BM_MANAGER (user_data), FALSE);
}
...
pk_authority_changed_cb (GObject *object, gpointer user_data)
{
	...
	g_signal_emit (BM_MANAGER (user_data), signals[CHECK_PERMISSIONS], 0);
}
...
get_perm_add_result (NMAuthChain *chain, GHashTable *results, const char *permission)
{
	...
	result = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, permission));
	if (result == BM_AUTH_CALL_RESULT_YES)
		g_hash_table_insert (results, (char *) permission, "yes");
	else if (result == BM_AUTH_CALL_RESULT_NO)
		g_hash_table_insert (results, (char *) permission, "no");
	else if (result == BM_AUTH_CALL_RESULT_AUTH)
		g_hash_table_insert (results, (char *) permission, "auth");
	else {
		bm_log_dbg (LOGD_CORE, "unknown auth chain result %d", result);
	}
}
                         ...
                         gpointer user_data)
{
	...
	priv->auth_chains = g_slist_remove (priv->auth_chains, chain);
	if (error) {
		bm_log_dbg (LOGD_CORE, "Permissions request failed: %s", error->message);
		ret_error = g_error_new (BM_MANAGER_ERROR,
		                         BM_MANAGER_ERROR_PERMISSION_DENIED,
		...
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_ENABLE_DISABLE_NETWORK);
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_SLEEP_WAKE);
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_ENABLE_DISABLE_WIFI);
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_ENABLE_DISABLE_WWAN);
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_USE_USER_CONNECTIONS);
		get_perm_add_result (chain, results, BM_AUTH_PERMISSION_NETWORK_CONTROL);
                              ...
                              DBusGMethodInvocation *context)
{
	...
	chain = bm_auth_chain_new (priv->authority, context, NULL, get_permissions_done_cb, self);
	g_assert (chain);
	priv->auth_chains = g_slist_append (priv->auth_chains, chain);

	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_ENABLE_DISABLE_NETWORK, FALSE);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_SLEEP_WAKE, FALSE);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_ENABLE_DISABLE_WIFI, FALSE);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_ENABLE_DISABLE_WWAN, FALSE);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_USE_USER_CONNECTIONS, FALSE);
	bm_auth_chain_add_call (chain, BM_AUTH_PERMISSION_NETWORK_CONTROL, FALSE);
}
...
gboolean
bm_manager_auto_user_connections_allowed (NMManager *self)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);

	return    priv->user_net_perm == BM_AUTH_CALL_RESULT_YES
	       && priv->user_con_perm == BM_AUTH_CALL_RESULT_YES;
}

static guint64
get_connection_timestamp (BMConnection *connection)
{
	if (bm_connection_get_scope (connection) == BM_CONNECTION_SCOPE_SYSTEM) {
		guint64 *ts_p;

		ts_p = (guint64 *) g_object_get_data (G_OBJECT (connection), BM_SYSCONFIG_SETTINGS_TIMESTAMP_TAG);
		...
		g_assert (s_con);
		return bm_setting_connection_get_timestamp (s_con);
	}
}
...
{
	BMConnection *a = BM_CONNECTION (pa);
	BMSettingConnection *con_a;
	BMConnection *b = BM_CONNECTION (pb);
	BMSettingConnection *con_b;
	guint64 ts_a, ts_b;

	con_a = (BMSettingConnection *) bm_connection_get_setting (a, BM_TYPE_SETTING_CONNECTION);
	g_assert (con_a);
	con_b = (BMSettingConnection *) bm_connection_get_setting (b, BM_TYPE_SETTING_CONNECTION);
	g_assert (con_b);

	if (bm_setting_connection_get_autoconnect (con_a) != bm_setting_connection_get_autoconnect (con_b)) {
		if (bm_setting_connection_get_autoconnect (con_a))
			return -1;
		return 1;
	}
...
/* Returns a GSList of referenced BMConnection objects, caller must
 * unref the connections in the list and destroy the list.
 */
GSList *
bm_manager_get_connections (NMManager *manager,
                            BMConnectionScope scope)
{
	NMManagerPrivate *priv;
	GSList *list = NULL;
	...
	if (scope == BM_CONNECTION_SCOPE_USER)
		...
		bm_log_err (LOGD_CORE, "unknown BMConnectionScope %d", scope);
	return list;
}

BMConnection *
bm_manager_get_connection_by_object_path (NMManager *manager,
                                          BMConnectionScope scope,
                                          const char *path)
{
	NMManagerPrivate *priv;
	...
	priv = BM_MANAGER_GET_PRIVATE (manager);
	if (scope == BM_CONNECTION_SCOPE_USER)
		connection = (BMConnection *) g_hash_table_lookup (priv->user_connections, path);
	else if (scope == BM_CONNECTION_SCOPE_SYSTEM)
		connection = (BMConnection *) g_hash_table_lookup (priv->system_connections, path);
	else
		bm_log_err (LOGD_CORE, "unknown BMConnectionScope %d", scope);
	return connection;
}
...
GPtrArray *
bm_manager_get_active_connections_by_connection (NMManager *manager,
                                                 BMConnection *connection)
{
...
void
bm_manager_start (NMManager *self)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	guint i;
	...
	for (i = 0; i < RFKILL_TYPE_MAX; i++) {
		...
		udev_state = bm_udev_manager_get_rfkill_state (priv->udev_mgr, i);
		update_rstate_from_rfkill (rstate, udev_state);

		if (rstate->desc) {
			bm_log_info (LOGD_RFKILL, "%s %s by radio killswitch; %s by state file",
				         rstate->desc,
...
handle_firmware_changed (gpointer user_data)
{
	...
	for (iter = priv->devices; iter; iter = iter->next) {
		BMDevice *candidate = BM_DEVICE (iter->data);
		BMDeviceState state = bm_device_get_state (candidate);

		if (   bm_device_get_firmware_missing (candidate)
		    && (state == BM_DEVICE_STATE_UNAVAILABLE)) {
			bm_log_info (LOGD_CORE, "(%s): firmware may now be available",
			             bm_device_get_iface (candidate));

			/* Re-set unavailable state to try bringing the device up again */
			bm_device_state_changed (candidate,
			                         BM_DEVICE_STATE_UNAVAILABLE,
			                         BM_DEVICE_STATE_REASON_NONE);
		}
	}
                      ...
                      gpointer user_data)
{
	...
	case G_FILE_MONITOR_EVENT_CHANGES_DONE_HINT:
		if (!priv->fw_changed_id) {
			...
			bm_log_info (LOGD_CORE, "kernel firmware directory '%s' changed",
			             KERNEL_FIRMWARE_DIR);
		}
		break;
	default:
                       ...
                       gpointer user_data)
{
	...
	request = bm_auth_chain_get_data (chain, "message");
	permission = bm_auth_chain_get_data (chain, "permission");
	prop = bm_auth_chain_get_data (chain, "prop");
	set_enabled = GPOINTER_TO_UINT (bm_auth_chain_get_data (chain, "enabled"));
             ...
             void *user_data)
{
	...
	if (!strcmp (propname, "WirelessEnabled")) {
		glib_propname = BM_MANAGER_WIRELESS_ENABLED;
		permission = BM_AUTH_PERMISSION_ENABLE_DISABLE_WIFI;
	} else if (!strcmp (propname, "WwanEnabled")) {
		glib_propname = BM_MANAGER_WWAN_ENABLED;
		permission = BM_AUTH_PERMISSION_ENABLE_DISABLE_WWAN;
	} else
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
	...
	if (uid > 0) {
		/* Otherwise validate the user request */
		chain = bm_auth_chain_new_raw_message (priv->authority, message, prop_set_auth_done_cb, self);
		g_assert (chain);
		priv->auth_chains = g_slist_append (priv->auth_chains, chain);
		bm_auth_chain_set_data (chain, "prop", g_strdup (glib_propname), g_free);
		bm_auth_chain_set_data (chain, "permission", g_strdup (permission), g_free);
		bm_auth_chain_set_data (chain, "enabled", GUINT_TO_POINTER (set_enabled), NULL);
		bm_auth_chain_set_data (chain, "message", dbus_message_ref (message), (GDestroyNotify) dbus_message_unref);
		bm_auth_chain_add_call (chain, permission, TRUE);
	} else {
		/* Yay for root */
                ...
                GError **error)
{
	...
	singleton = (NMManager *) g_object_new (BM_TYPE_MANAGER, NULL);
	g_assert (singleton);
	...
	g_signal_connect (priv->sys_settings, "notify::" BM_SYSCONFIG_SETTINGS_UNMANAGED_SPECS,
	                  G_CALLBACK (system_unmanaged_devices_changed_cb), singleton);
	g_signal_connect (priv->sys_settings, "notify::" BM_SETTINGS_SYSTEM_INTERFACE_HOSTNAME,
	...
	priv->bluez_mgr = bm_bluez_manager_get ();
...
{
	NMManager *manager = BM_MANAGER (object);
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
	GSList *iter;
	DBusGConnection *bus;
	DBusConnection *dbus_connection;
	...
	if (priv->disposed) {
		G_OBJECT_CLASS (bm_manager_parent_class)->dispose (object);
		return;
	}
	priv->disposed = TRUE;
	...
	g_slist_foreach (priv->auth_chains, (GFunc) bm_auth_chain_unref, NULL);
                            ...
                            gboolean enabled)
{
	...
	if (rstate->desc) {
		bm_log_dbg (LOGD_RFKILL, "(%s): setting radio %s by user",
		            rstate->desc,
		            enabled ? "enabled" : "disabled");
	}
	...
	if (priv->state_file) {
		if (!write_value_to_state_file (priv->state_file,
		                                ...
		                                &error)) {
			bm_log_warn (LOGD_CORE, "writing to state file %s failed: (%d) %s.",
			             priv->state_file,
			             error ? error->code : -1,
			             (error && error->message) ? error->message : "unknown");
			g_clear_error (&error);
...
{
	NMManager *self = BM_MANAGER (object);
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (self);
	...
	case PROP_WIRELESS_ENABLED:
		manager_radio_user_toggled (BM_MANAGER (object),
...
periodic_update_active_connection_timestamps (gpointer user_data)
{
	...
	for (i = 0; i < active->len; i++) {
		...
		req = bm_manager_get_act_request_by_path (manager, active_path, &device);
		if (device && bm_device_get_state (device) == BM_DEVICE_STATE_ACTIVATED)
			update_active_connection_timestamp (manager, device);
	}
...
static void
bm_manager_init (NMManager *manager)
{
	NMManagerPrivate *priv = BM_MANAGER_GET_PRIVATE (manager);
	DBusGConnection *g_connection;
	guint id, i;
	...
	priv->radio_states[RFKILL_TYPE_WLAN].prop = BM_MANAGER_WIRELESS_ENABLED;
	priv->radio_states[RFKILL_TYPE_WLAN].hw_prop = BM_MANAGER_WIRELESS_HARDWARE_ENABLED;
	priv->radio_states[RFKILL_TYPE_WLAN].desc = "WiFi";
	priv->radio_states[RFKILL_TYPE_WLAN].other_enabled_func = bm_manager_get_ipw_rfkill_state;
	priv->radio_states[RFKILL_TYPE_WLAN].rtype = RFKILL_TYPE_WLAN;

	priv->radio_states[RFKILL_TYPE_WWAN].user_enabled = TRUE;
	priv->radio_states[RFKILL_TYPE_WWAN].key = "WWANEnabled";
	priv->radio_states[RFKILL_TYPE_WWAN].prop = BM_MANAGER_WWAN_ENABLED;
	priv->radio_states[RFKILL_TYPE_WWAN].hw_prop = BM_MANAGER_WWAN_HARDWARE_ENABLED;
	priv->radio_states[RFKILL_TYPE_WWAN].desc = "WWAN";
	priv->radio_states[RFKILL_TYPE_WWAN].other_enabled_func = bm_manager_get_modem_enabled_state;
	...
	priv->modem_manager = bm_modem_manager_get ();
	...
	priv->vpn_manager = bm_vpn_manager_get ();
	...
	priv->aipd_proxy = dbus_g_proxy_new_for_name (g_connection,
	                                              BM_AUTOIP_DBUS_SERVICE,
	                                              "/",
	                                              BM_AUTOIP_DBUS_IFACE);
	if (priv->aipd_proxy) {
		...
		bm_log_warn (LOGD_AUTOIP4, "could not initialize avahi-autoipd D-Bus proxy");
	...
	if (priv->upower_proxy) {
		...
		bm_log_warn (LOGD_SUSPEND, "could not initialize UPower D-Bus proxy");
	...
	if (priv->authority) {
		...
		bm_log_warn (LOGD_CORE, "failed to create PolicyKit authority: (%d) %s",
		             error ? error->code : -1,
		             error && error->message ? error->message : "(unknown)");
		g_clear_error (&error);
	}
	...
	if (priv->fw_monitor) {
		priv->fw_monitor_id = g_signal_connect (priv->fw_monitor, "changed",
		                                        ...
		                                        manager);
		bm_log_info (LOGD_CORE, "monitoring kernel firmware directory '%s'.",
		             KERNEL_FIRMWARE_DIR);
	} else {
		bm_log_warn (LOGD_CORE, "failed to monitor kernel firmware directory '%s'.",
		             KERNEL_FIRMWARE_DIR);
	}
...
static void
bm_manager_class_init (NMManagerClass *manager_class)
{
	...
	g_object_class_install_property
		(object_class, PROP_WIRELESS_ENABLED,
		 g_param_spec_boolean (BM_MANAGER_WIRELESS_ENABLED,
		                       "WirelessEnabled",
		                       "Is wireless enabled",
		                       TRUE,
		                       G_PARAM_READWRITE));
	...
	g_object_class_install_property
		(object_class, PROP_WIRELESS_HARDWARE_ENABLED,
		 g_param_spec_boolean (BM_MANAGER_WIRELESS_HARDWARE_ENABLED,
	...
	g_object_class_install_property
		(object_class, PROP_WWAN_ENABLED,
		 g_param_spec_boolean (BM_MANAGER_WWAN_ENABLED,
		                       "WwanEnabled",
	...
	g_object_class_install_property
		(object_class, PROP_WWAN_HARDWARE_ENABLED,
		 g_param_spec_boolean (BM_MANAGER_WWAN_HARDWARE_ENABLED,
		                       "WwanHardwareEnabled",
	...
	g_object_class_install_property
		(object_class, PROP_ACTIVE_CONNECTIONS,
		 g_param_spec_boxed (BM_MANAGER_ACTIVE_CONNECTIONS,
		                     "Active connections",
		                     "Active connections",
	...
	g_object_class_install_property
		(object_class, PROP_HOSTNAME,
		 g_param_spec_string (BM_MANAGER_HOSTNAME,
		                      "Hostname",
		                      "Hostname",
		                      NULL,
	...
	g_object_class_install_property
		(object_class, PROP_SLEEPING,
		 g_param_spec_boolean (BM_MANAGER_SLEEPING,
		                       "Sleeping",
		                       "Sleeping",
		                       FALSE,
		                       G_PARAM_READABLE | BM_PROPERTY_PARAM_NO_EXPORT));
