...
crashlogger_get_backtrace (void)
{
	...
	{
		/* Child process */
		execl (LIBEXECDIR"/bm-crash-logger",
				LIBEXECDIR"/bm-crash-logger", NULL);
	}
...
nm_logging_backtrace (void)
{
	...
	/* Try to use gdb via bm-crash-logger if it exists, since
	 * we get much better information out of it.  Otherwise
	 * fall back to execinfo.
	 */
	if (stat (LIBEXECDIR"/bm-crash-logger", &s) == 0)
