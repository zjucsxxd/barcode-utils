...
/*
 * bm-online.c - is barcode manager running?
 *
 * Return values:
 *
 * 	0	: already running
 *	1	: offline or not running within given timeout
 *	2	: unspecified error
 *
				      ...
				      void *user_data G_GNUC_UNUSED)
{
	...
	if (!dbus_message_is_signal (message, BM_DBUS_INTERFACE, "StateChanged"))
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;

	if (!dbus_message_get_args (message, NULL, DBUS_TYPE_UINT32, &state, DBUS_TYPE_INVALID))
		return DBUS_HANDLER_RESULT_NOT_YET_HANDLED;

	if (state == BM_STATE_CONNECTED)
		g_main_loop_quit (loop);

	return DBUS_HANDLER_RESULT_HANDLED;
}

static BMState check_online (DBusConnection *connection)
{
	DBusMessage *message, *reply;
	DBusError error;
	dbus_uint32_t state;
	
	message = dbus_message_new_method_call (BM_DBUS_SERVICE, BM_DBUS_PATH,
						BM_DBUS_INTERFACE, "state");
	if (!message)
		exit (2);
...
int main (int argc, char *argv[])
{
	...
	GOptionEntry options[] = {
		...
		{"exit", 'x', 0, G_OPTION_ARG_NONE, &exit_no_bm, "Exit immediately if NetworkManager is not running or connecting", NULL},
	...
	g_option_context_set_summary (opt_ctx,
		"Waits for BarcodeManager to become available.");
	...
	if (state == BM_STATE_CONNECTED)
		return 0;
	if (exit_no_bm && (state != BM_STATE_CONNECTING))
		return 1;
