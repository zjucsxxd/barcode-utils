...
class BluetoothBox : DeviceBox {

    /* A Gtk.VBox which holds all of the widgets to control a bluetooth
      device. */
    ...
    private Barman.Service service = null;

    public BluetoothBox(Barman.Manager barman, string datadir) {
        this.set_spacing(12);
        this.datadir = datadir;
        this.barman = barman;

        this.barman.notify["bluetooth-state"].connect(this.bluetooth_state_changed);
        ...
        // Network Settings
        /// Creation
        this.builder = Utils.Gui.new_builder(datadir + "ui/bluetooth_box.ui");
        this.get_widgets();
        ...
        foreach (var service in services) {
			if (service.get_service_type() == Barman.ServiceType.BLUETOOTH) {
                /* we support only one bluetooth service for now */
        ...
        this.update_widget_states(this.barman.get_bluetooth_state());
    }
    ...
    private void update_widget_states(Barman.TechnologyState state) {
        ...
        case Barman.TechnologyState.ENABLED:
            ...
            status_text = ("Bluetooth is on but not connected to the Internet.");
            break;
        case Barman.TechnologyState.CONNECTED:
            device_editable = true;
            settings_editable = true;
            toggleswitch_state = true;
            status_text = ("Bluetooth is on and connected to the Internet.");
            break;
        case Barman.TechnologyState.AVAILABLE:
            device_editable = true;
            settings_editable = false;
            toggleswitch_state = false;
            status_text = ("The Bluetooth device is powered off.");
            break;
        case Barman.TechnologyState.OFFLINE:
            device_editable = true;
            settings_editable = false;
            toggleswitch_state = false;
            status_text = ("Bluetooth is offline.");
            break;
        }
    ...
    private void bluetooth_state_changed(ParamSpec p) {
        this.update_widget_states(this.barman.get_bluetooth_state());
    }
    ...
    private void on_toggleswitch_toggled() {
        if (this.toggleswitch.get_active()) {
            this.barman.enable_technology(Barman.TechnologyType.BLUETOOTH, null);
        } else {
            this.barman.disable_technology(Barman.TechnologyType.BLUETOOTH, null);
        }
    }
